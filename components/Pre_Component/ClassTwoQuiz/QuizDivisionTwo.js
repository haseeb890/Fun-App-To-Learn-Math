...
const boxWidth = 70;
export default function QuizDivisionTwo({ navigation }) {
  const ref = useRef();
  ...
  let number1 = Math.floor(Math.random(1) * 101);
  ...
  while (number1 % number2 !== 0) {
    number1 = Math.floor(Math.random(1) * 101);
    number2 = Math.floor(Math.random() * 11);
  }
  ...
  useEffect(() => {
    if (val.counter == 10) {
      setTimeout(() => {
        ...
        navigation.navigate("QuizDivi_Two_Result", {
