...
const boxWidth = 70;
export default function QuizAdd({ navigation, route }) {
  const ref = useRef();
  ...
  let randomResult1 = Math.floor(Math.random() * 51);
  ...
  useEffect(() => {
    if (val.counter == 5) {
      setTimeout(() => {
        ...
        navigation.navigate("QuizAdd_K_Result", { childresult: childresult });
      }, 1000);
